...
figure; step(sysd_wu);
%% Third feedback loop: zero static error
figure; rlocus(sysd_cl2);
ps2 = sort(pole(sysd_cl2));
R = zpk([ps2(3)], [1], 1, T_sample);
...
figure; step(sysd_wu);
%%
arduino = tcpclient('localhost', 6032, 'Timeout', 60);
%%
mode = CLASSICAL;
w = 0
set_mode_params(arduino, mode, w, [k1, k2, ps(2), 0.9*ps(2), k3, ps2(3)]);
...
ps = [0.8, 0.81]; L = place(sysd.A', sysd.C', ps); L=L';
y_ = step(sysd, ts);
...
mode = STATE_SPACE;
w = 0.0;
set_mode_params(arduino, mode, w, [K(1), K(2)])
input('press enter')
