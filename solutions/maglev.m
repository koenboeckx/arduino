...
%% identification
z0 = 5;
[sys, I0] = maglev_ident(arduino, z0);
...
%%
reset_system(arduino);
set_disturbance(arduino, 5.0);
set_mode_params(arduino, 20.0, 20, []);

input('press enter to continue')
Y = get_response(arduino, I0*.9, n_samples);
y = Y(2, :); u = Y(1, :);
%%
%sys.A(2, 1) = 2;
%sys.B(2) = -0.3;
sysd = c2d(sys, T_sample)
y_ = min(lsim(sysd, I0*.9-u, ts, [0, 0]) + z0, 10);
...
w = 0;
type = 1.; % 0 or 1
set_mode_params(arduino, mode, w, [k1, ps(2), prop_factor*ps(2), k2, ps2(2), type]);
input('press enter')
...
sysd = c2d(sys, T_sample)
ps = [0.65, 0.66]; L = place(sysd.A', sysd.C', ps); L=L';
y_ = step(sysd, ts);
